= RESTful Appointments Getting Started Guide
Zach Young;
:doctype: book
:icons: font
...
RESTful Appointments is a RESTful web service for creating and storing appointments. It uses hypermedia
...
== Running the service
RESTful Appointments is written using http://projects.spring.io/spring-boot[Spring Boot] which
...
[source,bash]
----
$ git clone https://github.com/zachariahyoung/docker-jpa
----
...
it to create a new appointment. As you saw above, the URI for working with notes is included as
a link when you perform a `GET` request against the root of the service:

include::{snippets}/index/1/http-response.adoc[]

To create a appointment, you need to execute a `POST` request to this URI including a JSON
payload containing the tracking number of the appointment:

include::{snippets}/creating-a-appointment/1/curl-request.adoc[]
...
include::{snippets}/creating-a-appointment/1/http-response.adoc[]

To work with the newly created appointment you use the URI in the `Location` header. For example,
you can access the appointment's details by performing a `GET` request:

include::{snippets}/creating-a-appointment/2/curl-request.adoc[]

This request will produce a response with the appointment's details in its body:

include::{snippets}/creating-a-appointment/2/http-response.adoc[]
